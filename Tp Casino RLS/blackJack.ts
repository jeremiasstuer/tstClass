...
export class BlackJack extends Juegos{
    ...
    public getCartasCrou():number[]{
        return this.cartasCroupier
    }

    public numbAleatorio() {return 1 + Math.floor(Math.random() * 11)}
    ...
    public validacionOtraCarta(){
        if(this.sumarCartas(this.getMisCartas())<= 21){
            ...
            console.log(`Cartas Croupier = ${this.getCartasCrou()} = ${this.sumarCartas(this.getCartasCrou())}`)
    ...
    public winLose(usuario,apuestaUs,premio){
        if(this.sumarCartas(this.getMisCartas())<=21 && this.sumarCartas(this.getCartasCrou()) <= 21){
            if(this.sumarCartas(this.getMisCartas()) > this.sumarCartas(this.getCartasCrou())){
                ...
                console.log(`El croupier se paso... Estos son sus puntos ${ this.sumarCartas(this.getCartasCrou())}`)
            }
    }
